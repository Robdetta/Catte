...
export class DeckMechanics {
  ...
  displayPlayerHand(playerId: string) {
    const playerInstance = this.playerManager.players.find(
      (player) => player.id === playerId,
    );
    if (playerInstance && playerInstance.hand) {
      // TODO: Logic to display each card in the player's hand at the bottom of the screen
      // This could involve creating new Phaser.GameObjects.Image instances for each card
    }
  }
